...
_NATURAL_BREAK = re.compile(
    r"\b(?:COBERTURA[S]?|EXCLUSAO(?:ES)?|FRANQUIA|ASSISTENCIA|SINISTRO|CONDICOES?\s+GERAIS?)\b",
    re.IGNORECASE,
)
...
def _extract_sections(text: str) -> List[Dict[str, str]]:
    ...
    markers = [
        ("coberturas", ["COBERTURAS", "COBERTURA"]),
        ("exclusoes", ["EXCLUSOES", "EXCLUSÕES", "RISCOS EXCLUIDOS", "RISCOS EXCLUÍDOS"]),
        ("franquia", ["FRANQUIA", "PARTICIPACAO DO SEGURADO", "PARTICIPAÇÃO DO SEGURADO"]),
        ("sinistro", ["SINISTRO", "AVISO DE SINISTRO", "REGULACAO DE SINISTRO", "REGULAÇÃO DE SINISTRO"]),
        ("assistencia", ["ASSISTENCIA", "ASSISTÊNCIA", "SERVICOS DE ASSISTENCIA", "SERVIÇOS DE ASSISTÊNCIA"]),
    ]
    ...
    if not positions:
        return [{"tipo": "conteudo_geral", "texto": text.strip()}]
    ...
    return sections if sections else [{"tipo": "conteudo_geral", "texto": text.strip()}]
...
def main(collection: str = "documentos_seguro_residencial", pasta_pdfs: str = "documentos"):
